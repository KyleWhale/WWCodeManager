...
- (void)play {
    ...
    /// Thanks @hootigger: https://github.com/changsanjiang/SJBaseCommonCode/pull/20/files
    ///
...
- (void)replay {
    ...
    [self seekToTime:kCMTimeZero completionHandler:^(BOOL finished) {
        ...
        if ( !self ) return;
        [self _postNotification:SJCodeShowDidReplayNotification];
        [self play];
    }];
}
...
- (void)report {
    [self _postNotification:SJCodeShowAssetStatusDidChangeNotification];
    [self _postNotification:SJCodeShowTimeControlStatusDidChangeNotification];
    [self _postNotification:SJCodeShowDurationDidChangeNotification];
    [self _postNotification:SJCodeShowPlayableDurationDidChangeNotification];
    [self _postNotification:SJCodeShowPlaybackTypeDidChangeNotification];
}
...
- (void)setAssetStatus:(SJAssetStatus)assetStatus {
    ...
    if ( _assetStatus == SJAssetStatusFailed ) {
        if ( _innerError != nil ) {
            NSLog(@"SJExecuteCode: %@", _innerError);
        }
        else if ( _avPlayer.error ) {
            NSLog(@"SJExecuteCode: %@", self.avPlayer.error);
        }
        else if ( _avPlayer.currentItem.error ) {
            NSLog(@"SJExecuteCode: %@", self.avPlayer.currentItem.error);
        }
    }
#endif
}
...
- (void)setIsPlaybackFinished:(BOOL)isPlaybackFinished {
    if ( isPlaybackFinished != _isPlaybackFinished ) {
        ...
        if ( isPlaybackFinished ) {
            [self _postNotification:SJCodeShowPlaybackDidFinishNotification];
        }
    }
}
...
- (void)setMuted:(BOOL)muted {
    ...
    [self _postNotification:SJCodeShowMutedDidChangeNotification];
}
- (BOOL)isMuted {
...
- (void)setVolume:(float)volume {
    ...
    [self _postNotification:SJCodeShowVolumeDidChangeNotification];
}
- (float)volume {
...
- (void)setRate:(float)rate {
    ...
    [self _postNotification:SJCodeShowRateDidChangeNotification];
}
...
- (void)_prepareToPlay {
    ...
    [self sj_observeWithNotification:AVPlayerItemFailedToPlayToEndTimeNotification target:playerItem usingBlock:^(SJExecuteCode *self, NSNotification * _Nonnull note) {
        [self _failedToPlayToEndTime:note];
    }];
    [self sj_observeWithNotification:AVPlayerItemDidPlayToEndTimeNotification target:playerItem usingBlock:^(SJExecuteCode *self, NSNotification * _Nonnull note) {
        [self _didPlayToEndTime:note];
    }];
    [self sj_observeWithNotification:AVPlayerItemNewAccessLogEntryNotification target:playerItem usingBlock:^(SJExecuteCode *self, NSNotification * _Nonnull note) {
...
#ifdef SJDEBUG
            if ( buffMilli < maxBuffMilli ) {
                printf("SJExecuteCode: 缓冲中...  进度: \t %ld \t %ld \n", (long)buffMilli, (long)maxBuffMilli);
            }
#endif
        }
    });
...
- (void)_presentationSizeDidChange {
    [self _postNotification:SJCodeShowPresentationSizeDidChangeNotification];
}

//
//  SJCommonCodeURLAsset+SJMTHFAdd.m
//  Project
//
...
#import "SJCommonCodeURLAsset+SJMTHFAdd.h"
#import <objc/message.h>

NS_ASSUME_NONNULL_BEGIN
@implementation SJCommonCodeURLAsset (SJMTHFAdd)
...
- (nullable instancetype)initWithOtherAsset:(SJCommonCodeURLAsset *)otherAsset playModel:(nullable __kindof SJPlayModel *)playModel {
    if ( !otherAsset ) return nil;
    self = [super init];
    if ( self ) {
        SJCommonCodeURLAsset *curr = otherAsset;
...
- (void)setOriginal:(SJCommonCodeURLAsset * _Nullable)original {
    objc_setAssociatedObject(self, @selector(original), original, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (nullable SJCommonCodeURLAsset *)original {
    return objc_getAssociatedObject(self, _cmd);
}

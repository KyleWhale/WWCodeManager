...
#import "SJCommonCode.h"
#import "UIView+SJAnimationAdded.h"
#import <objc/message.h>

#if __has_include(<SJBaseCommonCode/SJBaseCommonCode.h>)
#import <SJBaseCommonCode/SJBaseCommonCode.h>
#import <SJBaseCommonCode/SJBaseCommonCodeConst.h>
#import <SJBaseCommonCode/SJReachability.h>
...
@implementation SJEdgeControlLayer (SJCommonCodeExtended)
...
- (void)setClipsConfig:(nullable SJCommonCodeClipsConfig *)clipsConfig {
    objc_setAssociatedObject(self, @selector(clipsConfig), clipsConfig, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (SJCommonCodeClipsConfig *)clipsConfig {
    SJCommonCodeClipsConfig *config = objc_getAssociatedObject(self, _cmd);
    if ( config == nil ) {
        config = SJCommonCodeClipsConfig.alloc.init;
...
@interface SJCommonCode ()<SJSPDefinitionSwitchingControlLayerDelegate, SJMoreSettingControlLayerDelegate, SJNotReachableControlLayerDelegate, SJEdgeControlLayerDelegate>
...
@property (nonatomic, strong, readonly) SJSPDefinitionSwitchingInfoObserver *sj_switchingInfoObserver;
...
+ (instancetype)lightweightCode {
    SJCommonCode *commonCode = [[SJCommonCode alloc] _init];
    SJEdgeControlLayer *controlLayer = commonCode.defaultEdgeControlLayer;
    ...
    [commonCode.switcher switchControlLayerForIdentifier:SJControlLayer_Edge];
    return commonCode;
}
...
- (void)_definitionItemWasTapped:(SJEdgeControlButtonItem *)definitionItem {
    self.defaultVideoDefinitionSwitchingControlLayer.assets = self.definitionResources;
...
- (void)controlLayer:(SJSPDefinitionSwitchingControlLayer *)controlLayer didSelectAsset:(SJCommonCodeResource *)asset {
    SJCommonCodeResource *selected = self.resource;
...
- (SJSPDefinitionSwitchingControlLayer *)defaultVideoDefinitionSwitchingControlLayer {
    if ( _defaultVideoDefinitionSwitchingControlLayer == nil ) {
        _defaultVideoDefinitionSwitchingControlLayer = [[SJSPDefinitionSwitchingControlLayer alloc] initWithFrame:self.view.bounds];
...
- (SJSPDefinitionSwitchingInfoObserver *)sj_switchingInfoObserver {
    if ( _sj_switchingInfoObserver == nil ) {
        _sj_switchingInfoObserver = [self.definitionSwitchingInfo getObserver];
        __weak typeof(self) _self = self;
        _sj_switchingInfoObserver.statusDidChangeExeBlock = ^(SJSPDefinitionSwitchingInfo * _Nonnull info) {
            __strong typeof(_self) self = _self;
            if ( !self ) return ;
            ...
            switch ( info.status ) {
                ...
                case SJDefinitionSwitchStatusSwitching: {
                    [self.promptingPopupController show:[NSAttributedString sj_UIKitText:^(id<SJUIKitTextMakerProtocol>  _Nonnull make) {
                        make.append([NSString stringWithFormat:@"%@ %@", SJCommonCodeConfigurations.shared.localizedStrings.definitionSwitchingPrompt, info.switchingAsset.definition_fullName]);
                ...
                case SJDefinitionSwitchStatusFinished: {
                    [self.promptingPopupController show:[NSAttributedString sj_UIKitText:^(id<SJUIKitTextMakerProtocol>  _Nonnull make) {
                        make.append([NSString stringWithFormat:@"%@ %@", SJCommonCodeConfigurations.shared.localizedStrings.definitionSwitchSuccessfullyPrompt, info.currentPlayingAsset.definition_fullName]);
                        make.textColor(UIColor.whiteColor);
                    }]];
                }
                    break;
                case SJDefinitionSwitchStatusFailed: {
                    [self.promptingPopupController show:[NSAttributedString sj_UIKitText:^(id<SJUIKitTextMakerProtocol>  _Nonnull make) {
                        make.append(SJCommonCodeConfigurations.shared.localizedStrings.definitionSwitchFailedPrompt);
                        make.textColor(UIColor.whiteColor);
                    }]];
                }
                    break;
...
- (void)_observeNotifies {
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_switchControlLayerIfNeeded) name:SJCommonCodePlaybackTimeControlStatusDidChangeNotification object:self];
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_resumeOrStopTimeoutTimer) name:SJCommonCodePlaybackTimeControlStatusDidChangeNotification object:self];
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_switchControlLayerIfNeeded) name:SJCommonCodeAssetStatusDidChangeNotification object:self];
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_switchControlLayerIfNeeded) name:SJCommonCodePlaybackDidFinishNotification object:self];
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_configurationsDidUpdate) name:SJCommonCodeConfigurationsDidUpdateNotification object:nil];
...
- (void)_configurationsDidUpdate {
    if ( self.presentView.placeholderImageView.image == nil )
        self.presentView.placeholderImageView.image = SJCommonCodeConfigurations.shared.resources.placeholder;
    
    if ( _moreItem != nil )
        _moreItem.image = SJCommonCodeConfigurations.shared.resources.moreImage;
    
    if ( _clipsItem != nil )
        _clipsItem.image = SJCommonCodeConfigurations.shared.resources.clipsImage;
}
...
- (void)_updateAppearStateForClipsItemIfNeeded {
    if ( _clipsItem != nil ) {
        ...
        BOOL isHidden = (self.resource == nil) || !self.isFullscreen || isUnsupportedFormat || isPictureInPictureEnabled;
...
@implementation SJCommonCode (CommonSettings)
+ (void (^)(void (^ _Nonnull)(SJCommonCodeConfigurations * _Nonnull)))update {
    return SJCommonCodeConfigurations.update;
}

+ (void (^)(NSBundle * _Nonnull))setLocalizedStrings {
    return ^(NSBundle *bundle) {
        SJCommonCodeConfigurations.update(^(SJCommonCodeConfigurations * _Nonnull configs) {
            [configs.localizedStrings setFromBundle:bundle];
        });
    };
}

+ (void (^)(void (^ _Nonnull)(id<SJCommonCodeLocalizedStrings> _Nonnull)))updateLocalizedStrings {
    return ^(void(^block)(id<SJCommonCodeLocalizedStrings> strings)) {
        SJCommonCodeConfigurations.update(^(SJCommonCodeConfigurations * _Nonnull configs) {
...
+ (void (^)(void (^ _Nonnull)(id<SJCommonCodeControlLayerResources> _Nonnull)))updateResources {
    return ^(void(^block)(id<SJCommonCodeControlLayerResources> resources)) {
        SJCommonCodeConfigurations.update(^(SJCommonCodeConfigurations * _Nonnull configs) {
...
#pragma mark -
@implementation SJCommonCode (SJExtendedVideoDefinitionSwitchingControlLayer)

- (void)setDefinitionResources:(nullable NSArray<SJCommonCodeResource *> *)definitionResources {
    objc_setAssociatedObject(self, @selector(definitionResources), definitionResources, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (nullable NSArray<SJCommonCodeResource *> *)definitionResources {
...
@implementation SJCommonCode (RotationOrFitOnScreen)
...
@implementation SJCommonCode (SJExtendedControlLayerSwitcher)

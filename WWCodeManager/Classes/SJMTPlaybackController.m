...
- (instancetype)initWithDefinitionMediaPlayer:(id<SJCodeShow>)definitionMediaPlayer
                    definitionMediaPlayerView:(UIView<SJCodeShowView> *)definitionMediaPlayerView
                                currentPlayer:(id<SJCodeShow>)currentPlayer
                            currentPlayerView:(UIView<SJCodeShowView> *)currentPlayerView
                            completionHandler:(void(^)(SJDefinitionMediaPlayerLoader *loader, BOOL isFinished))completionHandler;

@property (nonatomic, strong, readonly, nullable) id<SJCodeShow> definitionMediaPlayer;
@property (nonatomic, strong, readonly, nullable) UIView<SJCodeShowView> *definitionMediaPlayerView;
@property (nonatomic, strong, readonly, nullable) id<SJCodeShow> currentPlayer;
@property (nonatomic, strong, readonly, nullable) UIView<SJCodeShowView> *currentPlayerView;
- (void)cancel;
@end


@interface SJCodeShowTimeObserverItem : NSObject
- (instancetype)initWithInterval:(NSTimeInterval)interval player:(__weak id<SJCodeShow>)player currentTimeDidChangeExeBlock:(nonnull void (^)(NSTimeInterval time))currentTimeDidChangeExeBlock playableDurationDidChangeExeBlock:(nonnull void (^)(NSTimeInterval time))playableDurationDidChangeExeBlock durationDidChangeExeBlock:(nonnull void (^)(NSTimeInterval time))durationDidChangeExeBlock;
- (void)invalidate;
- (void)stop;
@end

@implementation SJCodeShowTimeObserverItem {
...
- (instancetype)initWithInterval:(NSTimeInterval)interval player:(__weak id<SJCodeShow>)player currentTimeDidChangeExeBlock:(nonnull void (^)(NSTimeInterval))currentTimeDidChangeExeBlock playableDurationDidChangeExeBlock:(nonnull void (^)(NSTimeInterval))playableDurationDidChangeExeBlock durationDidChangeExeBlock:(nonnull void (^)(NSTimeInterval))durationDidChangeExeBlock {
    ...
    if ( self ) {
        ...
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(resumeOrPause) name:SJCodeShowTimeControlStatusDidChangeNotification object:player];
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(durationDidChange) name:SJCodeShowDurationDidChangeNotification object:_player];
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(playableDurationDidChange) name:SJCodeShowPlayableDurationDidChangeNotification object:_player];
    }
    return self;
}
...
@interface SJCodeShowContentView : UIView
@property (nonatomic, strong, nullable) UIView <SJCodeShowView> *view;
@end

@implementation SJCodeShowContentView
- (void)setView:(nullable UIView<SJCodeShowView> *)view {
...
@interface SJMTPlaybackController () {
    SJCodeShowContentView *_playerView;
}
...
@property (nonatomic, strong, nullable) id<SJCodeShow> currentPlayer;
...
@property (nonatomic, strong, nullable) SJCommonCodeResource *definitionMedia;
@end

@implementation SJMTPlaybackController
...
- (void)prepareToPlay {
    ...
    SJCommonCodeResource *media = _media;
    __weak typeof(self) _self = self;
    [self playerWithMedia:media completionHandler:^(id<SJCodeShow>  _Nullable player) {
        __strong typeof(_self) self = _self;
        if ( !self ) return;
...
- (void)switchVideoDefinition:(SJCommonCodeResource *)media {
    // clean
    ...
    [self playerWithMedia:media completionHandler:^(id<SJCodeShow>  _Nullable player) {
        ...
        id<SJCodeShow> definitionMediaPlayer = player;
        UIView<SJCodeShowView> *definitionMediaPlayerView = [self playerViewWithPlayer:player];
        
        id<SJCodeShow> currentPlayer = self.currentPlayer;
        UIView<SJCodeShowView> *currentPlayerView = self.currentPlayerView;
        ...
        self.definitionMediaPlayerLoader = [SJDefinitionMediaPlayerLoader.alloc initWithDefinitionMediaPlayer:definitionMediaPlayer definitionMediaPlayerView:definitionMediaPlayerView currentPlayer:currentPlayer currentPlayerView:currentPlayerView completionHandler:^(SJDefinitionMediaPlayerLoader * _Nonnull loader, BOOL isFinished) {
            ...
            if ( !isFinished ) {
                ...
                id<SJCodeShow> player = loader.definitionMediaPlayer;
                SJCommonCodeResource *newMedia = media;
                [self replaceMediaForDefinitionMedia:newMedia];
                
                id<SJCodeShow> oldPlayer = self.currentPlayer;
                id<SJCodeShow> newPlayer = player;
...
- (SJCodeShowContentView *)codeView {
    if ( _playerView == nil ) {
        _playerView = [SJCodeShowContentView.alloc initWithFrame:CGRectZero];
    }
    return _playerView;
}
...
- (void)setMedia:(nullable SJCommonCodeResource *)media {
    if ( _media != nil ) [self stop];
    _media = media;
}

- (void)replaceMediaForDefinitionMedia:(SJCommonCodeResource *)definitionMedia {
...
- (void)setCurrentPlayer:(nullable id<SJCodeShow>)currentPlayer {
...
- (void)setCurrentPlayerView:(__kindof UIView<SJCodeShowView> * _Nullable)currentPlayerView {
    currentPlayerView.spGravity = self.spGravity;
    _playerView.view = currentPlayerView;
}
- (nullable __kindof UIView<SJCodeShowView> *)currentPlayerView {
...
- (void)showLog_TimeControlStatus {
    ...
    switch ( status ) {
        case SJPlaybackTimeControlStatusPaused: {
            statusStr = [NSString stringWithFormat:@"SJMTPlaybackController<%p>.TimeControlStatus.Paused\n", self];
        }
            break;
        case SJPlaybackTimeControlStatusWaitingToPlay: {
            ...
            statusStr = [NSString stringWithFormat:@"SJMTPlaybackController<%p>.TimeControlStatus.WaitingToPlay(Reason: %@)\n", self, reasonStr];
        }
            break;
        case SJPlaybackTimeControlStatusPlaying: {
            statusStr = [NSString stringWithFormat:@"SJMTPlaybackController<%p>.TimeControlStatus.Playing\n", self];
        }
            break;
    }
...
- (void)showLog_AssetStatus {
    ...
    switch ( status ) {
        case SJAssetStatusUnknown:
            statusStr = [NSString stringWithFormat:@"SJMTPlaybackController<%p>.assetStatus.Unknown\n", self];
            break;
        case SJAssetStatusPreparing:
            statusStr = [NSString stringWithFormat:@"SJMTPlaybackController<%p>.assetStatus.Preparing\n", self];
            break;
        case SJAssetStatusReadyToPlay:
            statusStr = [NSString stringWithFormat:@"SJMTPlaybackController<%p>.assetStatus.ReadyToPlay\n", self];
            break;
        case SJAssetStatusFailed:
            statusStr = [NSString stringWithFormat:@"SJMTPlaybackController<%p>.assetStatus.Failed\n", self];
            break;
    }
...
- (void)_definitionMedia:(id<SJMTModelProtocol>)media switchStatusDidChange:(SJDefinitionSwitchStatus)status {
    ...
    printf("SJMTPlaybackController<%p>.switchStatus = %s\n", self, str);
#endif
}

- (void)_addPeriodicTimeObserver {
    __weak typeof(self) _self = self;
    _periodicTimeObserver = [SJCodeShowTimeObserverItem.alloc initWithInterval:_periodicTimeInterval player:self.currentPlayer currentTimeDidChangeExeBlock:^(NSTimeInterval time) {
        __strong typeof(_self) self = _self;
        if ( !self ) return;
...
- (void)_initObservations {
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(playerAssetStatusDidChange:) name:SJCodeShowAssetStatusDidChangeNotification object:nil];
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(playerTimeControlStatusDidChange:) name:SJCodeShowTimeControlStatusDidChangeNotification object:nil];
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(playbackDidFinish:) name:SJCodeShowPlaybackDidFinishNotification object:nil];
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(playerPresentationSizeDidChange:) name:SJCodeShowPresentationSizeDidChangeNotification object:nil];
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(playerViewReadyForDisplay:) name:SJCodeShowViewReadyForDisplayNotification object:nil];
    [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(playerDidReplay:) name:SJCodeShowDidReplayNotification object:nil];
...
- (instancetype)initWithDefinitionMediaPlayer:(id<SJCodeShow>)definitionMediaPlayer
    ...
    if ( self ) {
        ...
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_statusDidChange) name:SJCodeShowAssetStatusDidChangeNotification object:definitionMediaPlayer];
        [NSNotificationCenter.defaultCenter addObserver:self selector:@selector(_statusDidChange) name:SJCodeShowViewReadyForDisplayNotification object:definitionMediaPlayerView];
...
NSNotificationName const SJCodeShowAssetStatusDidChangeNotification = @"SJCodeShowAssetStatusDidChangeNotification";
NSNotificationName const SJCodeShowTimeControlStatusDidChangeNotification = @"SJCodeShowTimeControlStatusDidChangeNotification";
NSNotificationName const SJCodeShowPresentationSizeDidChangeNotification = @"SJCodeShowPresentationSizeDidChangeNotification";
NSNotificationName const SJCodeShowPlaybackDidFinishNotification = @"SJCodeShowPlaybackDidFinishNotification";
NSNotificationName const SJCodeShowDidReplayNotification = @"SJCodeShowDidReplayNotification";
NSNotificationName const SJCodeShowDurationDidChangeNotification = @"SJCodeShowDurationDidChangeNotification";
NSNotificationName const SJCodeShowPlayableDurationDidChangeNotification = @"SJCodeShowPlayableDurationDidChangeNotification";
NSNotificationName const SJCodeShowRateDidChangeNotification = @"SJCodeShowRateDidChangeNotification";
NSNotificationName const SJCodeShowVolumeDidChangeNotification = @"SJCodeShowVolumeDidChangeNotification";
NSNotificationName const SJCodeShowMutedDidChangeNotification = @"SJCodeShowMutedDidChangeNotification";


NSNotificationName const SJCodeShowViewReadyForDisplayNotification = @"SJCodeShowViewReadyForDisplayNotification";
NSNotificationName const SJCodeShowPlaybackTypeDidChangeNotification = @"SJCodeShowPlaybackTypeDidChangeNotification";

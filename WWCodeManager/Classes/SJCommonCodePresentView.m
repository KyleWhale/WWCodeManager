...
@interface SJCommonCodePresentView ()<UIGestureRecognizerDelegate>
...
@implementation SJCommonCodePresentView
...
- (void)handleSingleTap:(UITouch *)tap {
    if ( ![self _isGestureSupported:SJBFCodeGestureTypeMask_SingleTap ] )
        return;
    
    CGPoint location = [tap locationInView:self];
    if ( _gestureRecognizerShouldTrigger && _gestureRecognizerShouldTrigger(self, SJBFCodeGestureType_SingleTap, location) ) {
...
- (void)handleDoubleTap:(UITouch *)tap {
    if ( ![self _isGestureSupported:SJBFCodeGestureTypeMask_DoubleTap] )
        return;
    
    CGPoint location = [tap locationInView:self];
    if ( _gestureRecognizerShouldTrigger && _gestureRecognizerShouldTrigger(self, SJBFCodeGestureType_DoubleTap, location) ) {
...
- (void)cancelGesture:(SJBFCodeGestureType)type {
    ...
    switch ( type ) {
        default: break;
        case SJBFCodeGestureType_Pan:
            gesture = _pan;
            break;
        case SJBFCodeGestureType_Pinch:
            gesture = _pinch;
            break;
        case SJBFCodeGestureType_LongPress:
            gesture = _longPress;
            break;
    }
...
- (void)_setupViews {
    self.supportedGestureTypes = SJBFCodeGestureTypeMask_Default;
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer {
    SJBFCodeGestureType type = SJBFCodeGestureType_Pan;
    if ( gestureRecognizer == _pinch )
        type = SJBFCodeGestureType_Pinch;
    else if ( gestureRecognizer == _longPress )
        type = SJBFCodeGestureType_LongPress;
    
    switch ( type ) {
        default: break;
        case SJBFCodeGestureType_Pan: {
            if ( ![self _isGestureSupported:SJBFCodeGestureTypeMask_Pan] )
                return NO;
            ...
            if ( _movingDirection == SJPanGestureMovingDirection_H && ![self _isGestureSupported:SJBFCodeGestureTypeMask_Pan_H] )
                return NO;
            
            if ( _movingDirection == SJPanGestureMovingDirection_V && ![self _isGestureSupported:SJBFCodeGestureTypeMask_Pan_V] )
                return NO;
            ...
            break;
        case SJBFCodeGestureType_Pinch: {
            if ( ![self _isGestureSupported:SJBFCodeGestureTypeMask_Pinch] )
                return NO;
        }
            break;
        case SJBFCodeGestureType_LongPress: {
            if ( ![self _isGestureSupported:SJBFCodeGestureTypeMask_LongPress] )
                return NO;
        }
            break;
    }
...
- (BOOL)_isGestureSupported:(SJBFCodeGestureTypeMask)type {

...
@interface SJBaseCommonCode : NSObject
+ (NSString *)version;
+ (instancetype)shared;
- (instancetype)init;
...
///
/// \code
/// _player = SJBaseCommonCode.shared;
/// _player.view.frame = ...;
/// [self.view addSubview:_player.view];
/// \endcode
///
@property (nonatomic, strong, readonly) __kindof UIView *view;
@property (nonatomic, weak, nullable) id <SJCommonCodeControlLayerDataSource> controlLayerDataSource;
@property (nonatomic, weak, nullable) id <SJCommonCodeControlLayerDelegate> controlLayerDelegate;
@end


@interface SJBaseCommonCode (AudioSession)
///
...
@interface SJBaseCommonCode (Placeholder)
///
/// 播放画面显示层
///
/// \code
...
/// \endcode
///
@property (nonatomic, strong, readonly) UIView<SJCommonCodePresentView> *presentView;
...
@interface SJBaseCommonCode (VideoFlipTransition)
...
@interface SJBaseCommonCode (Playback)<SJCommonCodePlaybackControllerDelegate>
...
///         了解更多请前往协议头文件查看
///
@property (nonatomic, strong, null_resettable) __kindof id<SJCommonCodePlaybackController> playbackController;
...
///         使用URL及相关的视图信息进行初始化
///
@property (nonatomic, strong, nullable) SJCommonCodeResource *resource;
...
///
/// 当您想在后台播放视频时:
/// 1. 需要设置 commonCode.pausedInBackground = NO; (该值默认为YES, 即App进入后台默认暂停).
...
/// - 当`block == nil`时, 默认为执行.
///
@property (nonatomic, copy, nullable) BOOL(^canPlayAnAsset)(__kindof SJBaseCommonCode *player);
...
/// - 当`block == nil`时, 默认为执行.
///
@property (nonatomic, copy, nullable) BOOL(^canSeekToTime)(__kindof SJBaseCommonCode *player);
...
/// 当前清晰度切换的信息
///
@property (nonatomic, strong, readonly) SJSPDefinitionSwitchingInfo *definitionSwitchingInfo;
...
@interface SJBaseCommonCode (DeviceVolumeAndBrightness)
...
/// v1.3.0 新增
/// 请在适当的时候调用这些方法
@interface SJBaseCommonCode (Life)
...
@interface SJBaseCommonCode (Network)
...
@interface SJBaseCommonCode (Popup)
...
/**
 ...
 LongPressGesture
 长按手势
 当用户长按播放器时, 将加速播放. 该手势默认不会启用, 如需开启请设置`player.gestureController.supportedGestureTypes = SJBFCodeGestureTypeMask_LongPress | 其他支持的手势;`
 */
@interface SJBaseCommonCode (Gesture)

///
/// 手势控制
///
///         如果想自己设置支持的手势类型, 可以`player.gestureController.supportedGestureTypes = SJBFCodeGestureTypeMask_SingleTap | 其他支持的手势;`
///         了解更多请前往头文件查看
///
...
///
@property (nonatomic, copy, nullable) BOOL(^gestureRecognizerShouldTrigger)(__kindof SJBaseCommonCode *player, SJBFCodeGestureType type, CGPoint location);
...
@interface SJBaseCommonCode (ControlLayer)
///
/// 对控制层显示/隐藏的控制
///
...
///
@property (nonatomic, copy, nullable) BOOL(^canAutomaticallyDisappear)(__kindof SJBaseCommonCode *player);
...
/// 全屏或小屏, 不会触发旋转
///
@interface SJBaseCommonCode (FitOnScreen)
...
/// 全屏或小屏, 会触发旋转
///
@interface SJBaseCommonCode (Rotation)
...
///
@property (nonatomic, copy, nullable) BOOL(^shouldTriggerRotation)(__kindof SJBaseCommonCode *player);
...
@interface SJBaseCommonCode (Screenshot)

// - Presentation Size -

@property (nonatomic, copy, nullable) void(^presentationSizeDidChangeExeBlock)(__kindof SJBaseCommonCode *commonCode);
...
@interface SJBaseCommonCode (Export)
...
@interface SJBaseCommonCode (ScrollView)
...
///
///     // more
/// #import <SJBaseCommonCode/SJSmallViewFloatingController.h>
///
...
@property (nonatomic, copy, nullable) void(^playerViewWillAppearExeBlock)(__kindof SJBaseCommonCode *commonCode);
@property (nonatomic, copy, nullable) void(^playerViewWillDisappearExeBlock)(__kindof SJBaseCommonCode *commonCode);
@end


#pragma mark - 字幕

@interface SJBaseCommonCode (Subtitle)
///
/// 字幕管理
///
/// \code
///
/// #import <SJBaseCommonCode/SJCommonCodeResource+SJSubtitlesAdd.h>
///
///     // 1. 创建资源
...
@interface SJBaseCommonCode (Danmaku)
...
/// \code
///
/// #import <SJBaseCommonCode/SJDanmakuItem.h>
///
///     // 创建一条弹幕
...
@interface SJBaseCommonCode (Watermark)

///
/// 添加水印视图
///
/// \code
/// #import <SJBaseCommonCode/SJWatermarkView.h>
///
...
@interface SJBaseCommonCode (Deprecated)
